...
import { PDFDocument, rgb, PDFName, type PDFFont } from "pdf-lib";
import fontkit from "@pdf-lib/fontkit";
import sharp from "sharp";
import type { ContactData, Manager, Project, ProjectLayout, ProjectPackage, ShopConfig } from "../shop/shopData.js";
...
function wrapTextByWidth(text: string, maxWidth: number, font: PDFFont, fontSize: number): string[] {
...
async function getFont(doc: PDFDocument, shopRootDir: string): Promise<PDFFont> {
...
export async function generateCommercialOfferPdf(params: {
  ...
  const templatePath = path.join(
    shopConfig.shopRootDir,
    project.name, // e.g. "Проект Дюна"
  ...
  const replaceFieldWithText = (fieldName: string, text: string, pageIndex: number, fontSize: number) => {
    try {
  ...
  ['Стоимость', 'Менеджер', 'Заказчик'].forEach(fieldName => {
    try {
      const field = form.getTextField(fieldName);
      const withRemove = field as unknown as { removeWidget: (w: unknown) => void };
